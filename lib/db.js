...
module.exports = class DB {
  ...
  constructor(uploadDir, store) {
    ...
    // Soft-expire files: stop them being served via the public link, but
    // keep them on disk so the admin can still access them indefinitely.
    const gc = () => {
      let sid,f,expires;
      for (sid of Object.keys(this.db)) {
        for (f of this.db[sid]) {
          if (f.metadata.expired) continue; // already soft-expired, nothing to do
          ...
          if(expires <= 0) {
            debug(`Expired (soft) ${sid}++${f.key}`);
            this.expire(sid, f.key).catch(e => console.error(e));
          }
        }
      }
    };
  ...
  /**
   * Soft-expire a file: it stops being reachable via the public share link,
   * but the file itself stays on disk and in the db, so the admin can still
   * see and download it at any time via /admin.
   */
  async expire(sid, key) {
    if(!this.initialized) throw new Error('DB not initialized');
    const bucket = this.get(sid);
    const data = bucket && bucket.find(item => item.key === key);
    if(!data || data.metadata.expired) return;
    debug(`Expire (soft) ${sid}++${key}`);
