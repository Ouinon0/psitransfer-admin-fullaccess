...
app.get(`${ config.baseUrl }admin/data.json`, (req, res, next) => {
  ...
  Object.values(result).forEach(bucket => {
    bucket.forEach(file => {
      ...
      // Admin gets a direct download link for every file, protected or not:
      // the /files/:fid route never enforces the bucket password anyway,
      // it only gates it in the download-list page.
      file.url = `${ config.baseUrl }files/${ file.metadata.sid }++${ file.key }`;
    });
  });
